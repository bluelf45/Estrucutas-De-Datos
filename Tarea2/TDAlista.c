...
void free2(tLista* listaD, tLista* listaU, int byte){
    ...
    printf("Bloque de %d bytes liberado", bytes);
}
...
int malloc2(tLista* listaD, tLista* listaU, int bytesize){
    ...
    while(listaD->curr != NULL){
        aval = listaD->curr->data2 - listaD->curr->data1; //Calcula el espacio disponible
        ...
        if(aval >= bytesize){
            ...
            printf("Bloque de %i bytes asignado a partir del byte %i", bytesize, listaD->curr->data1);
            listaU->listSize++;
            listaD->curr->data1 = inicio + aval + 1; //Cambia el tamaño del nodo en la lista 1.
    ...
    printf("Bloque de %i bytes NO puede ser asignado",bytesize);
    }
}
